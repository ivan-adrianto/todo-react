...
const Done = ({ data, isLoading, isError, getData, updateTodo }) => {
    ...
    return (
        <div>
            ...
            {data && data.filter(chosen => chosen.status === 1).sort(function(a, b){return new Date (b.createdAt) - new Date (a.createdAt)}).map(todos =>
                <div className='d-flex'>
                    <input type="checkbox" className='check' id={`${todos.title}&${todos.description}&${todos.id}&${todos.createdAt}&${todos.status}`} onChange={e => handleCheckBox(e)} checked={true}/>
...
export default connect(stateProps, dispatchProps)(Done);